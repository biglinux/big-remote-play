    ...
    def detect_steam(self):
        ...
        if vdf_path.exists():
            try:
                ...
                _log.error(f"Error reading Steam library: {e}")
                pass
        ...
        if games_dir.exists():
            for p in games_dir.glob("*.yml"):
                try:
                    ...
                    if name:
                        games.append({"name": name, "id": slug, "platform": "Lutris", "cmd": f"lutris lutris:rungame/{slug}", "icon": "lutris"})
                except Exception:
                    pass
        return games
        ...
        for p in possible_files:
            if p.exists():
                try:
                    ...
                    _log.error(f"Error reading Heroic {p}: {e}")
                    pass
