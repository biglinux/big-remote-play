...
class AudioManager:
    ...
    def get_passive_sinks(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            res = subprocess.run(["pactl", "list", "sinks"], capture_output=True, text=True, timeout=10)
            if res.returncode != 0:
                return []
    ...
    def enable_streaming_audio(self, host_sink: str, guest_only: bool = False) -> bool:
        ...
        if not host_sink or self.is_virtual(host_sink):
            ...
            if hardware_devices:
                ...
                _log.info(f"Host sink was virtual or null, fallback to hardware: {host_sink}")
            else:
                _log.error("ERROR: No hardware audio device found.")
                return False
            ...
            try:
                ...
                for app in apps:
                    ...
                    if self.is_virtual(app.get("sink_name", "")):
                        _log.info(f"Restoring {app.get('name')} to {host_sink}")
                        self.move_app(app["id"], host_sink)
            except Exception as e:
                _log.error(f"Error restoring apps to hardware: {e}")
    ...
    def get_apps(self) -> List[Dict]:
        ...
        try:
            ...
            res = subprocess.run(["pactl", "list", "sink-inputs"], capture_output=True, text=True, timeout=10)
            current = {}
