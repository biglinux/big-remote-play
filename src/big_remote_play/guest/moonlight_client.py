...
class MoonlightClient:
    ...
    def pair(self, host_ip: str, on_pin_callback: Callable[[str], None] | None = None) -> bool:
        ...
        try:
            ...
            if self.logger:
                self.logger.info(f"Starting pair with {host_ip} (target: {target_ip}): {' '.join(cmd)}")
            ...
            if self.logger:
                self.logger.error(f"Pairing exception: {e}")
            return False
    ...
    def list_apps(self, host_ip):
        ...
        try:
            ...
            if ":" in target_ip:
                ...
                try:
                    subprocess.run(["ip", "-6", "neigh", "flush", "all"], capture_output=True, timeout=1)
                except Exception:
                    pass
