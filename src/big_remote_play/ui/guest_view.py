...
class GuestView(Gtk.Box):
    ...
    def setup_ui(self):
        ...
        help_btn.update_property([Gtk.AccessibleProperty.LABEL], [_("Shortcuts & Instructions")])
        help_btn.connect("clicked", lambda b: self.show_shortcuts_dialog())

        switcher_bar = Gtk.CenterBox()
        switcher_bar.set_center_widget(switcher)
        switcher_bar.set_end_widget(help_btn)

        self.switcher_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=12)
        self.switcher_box.append(switcher_bar)
        self.switcher_box.append(self.method_stack)

        settings_group = Adw.PreferencesGroup()
        settings_group.set_title(_("Client Settings"))
        ...
        for r in ["720p", "1080p", "1440p", "4K", _("Custom")]:
            res_model.append(r)
        ...
        self.fps_row.set_subtitle(_("Video smoothness"))
        fps_model = Gtk.StringList()
        for f in ["30 FPS", "60 FPS", "120 FPS", _("Custom")]:
            fps_model.append(f)
        ...
        bitrate_row.set_subtitle(_("Adjust bandwidth (0.5 - 150 Mbps)"))
        ...
        detect_btn = Gtk.Button(label=_("Detect"))
        ...
        self.display_mode_row.set_subtitle(_("How the window will be displayed"))
        disp_model = Gtk.StringList()
        for d in [_("Borderless Window"), _("Fullscreen"), _("Windowed")]:
            disp_model.append(d)
    ...
    def update_ui_state(self):
        ...
        if hasattr(self, "apply_settings_btn"):
            self.apply_settings_btn.set_visible(c)
    ...
    def discover_hosts(self):
        ...
        spinner.start()
        lbl = Gtk.Label(label=_("Searching for hosts..."))
    ...
    def create_host_row_custom(self, host):
        ...
        n = Gtk.Label(label=host["name"])
        n.set_halign(Gtk.Align.START)
        n.add_css_class("heading")
        i = Gtk.Label(label=host["ip"])
        ...
        gesture.connect("pressed", lambda g, n, x, y: radio.set_active(True))
    ...
    def show_pin_dialog(self, pin):
        ...
        self.pin_dialog.add_response("cancel", _("Cancel"))
        self.pin_dialog.present()

    def close_pin_dialog(self):
        (self.pin_dialog.close() if self.pin_dialog else None)
        self.pin_dialog = None
    ...
    def show_pairing_dialog(self, host_ip, pin=None, on_confirm=None, hostname=None):
        ...
        if pin:
            body = f'<span size="xx-large" weight="bold" color="#3584e4">{pin}</span>\n\n' + body
        ...
        self.pairing_dialog.set_resizable(True)
        self.pairing_dialog.add_response("ok", _("OK"))
        self.pairing_dialog.set_response_appearance("ok", Adw.ResponseAppearance.SUGGESTED)

        def on_resp(dlg, resp):
            if resp == "ok" and on_confirm:
                on_confirm()

        self.pairing_dialog.connect("response", on_resp)
    ...
    def save_guest_settings(self):
        ...
        if f_idx == 0:
            ...
            fps = getattr(self, "custom_fps_val", "60")
    ...
    def reset_to_defaults(self):
        ...
        self.audio_row.set_active(True)
        self.hw_decode_row.set_active(True)
        self.custom_resolution_val = self.custom_fps_val = ""
        self.show_toast(_("Restored"))
        self.save_guest_settings()
    ...
    def open_advanced_client_settings(self, _widget=None):
        ...
        # Refresh the inline quick-settings after the advanced sheet closes.
        win.connect("close-request", lambda w: (self.load_guest_settings(), False)[1])
        win.present()
    ...
    def show_shortcuts_dialog(self):
        ...
        row_trouble.set_subtitle(
            _(
                "If shortcuts like F1/F2/F3 are not switching screens:\n"
