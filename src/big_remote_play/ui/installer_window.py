        ...
        for t in terms:
            if shutil.which(t[0]):
                try:
                    ...
                    continue
        if started:
            self.status_label.set_text(_("Running in external terminal. Restart after completion."))
            ...
            self.status_label.set_text(_("Error: No terminal detected."))
            self.textview.get_buffer().set_text(_("Execute manually:\n{}").format(cmd))
    ...
    def start_installation(self):
        ...
        try:
            self.terminal.spawn_async(
                Vte.PtyFlags.DEFAULT, None, ["yay", "-S", "--noconfirm", "--needed", "sunshine", "moonlight-qt"], None, GLib.SpawnFlags.SEARCH_PATH, None, -1, Gio.Cancellable(), on_spawn_done, None
            )
        except Exception as e:
            self.status_label.set_text(_("Embedded terminal error: {}. Trying external...").format(e))
            GLib.idle_add(self.start_external_installation)
