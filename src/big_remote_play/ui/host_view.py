...
class HostView(Gtk.Box):
    ...
    def setup_ui(self):
        ...
        self.game_mode_row.set_title(_("Game Mode"))
        self.game_mode_row.set_subtitle(_("Select game source"))
        ...
        for p in [_("Automatic"), "Wayland", "X11", _("KMS (Direct)")]:
        ...
        tips.append(_tip("dialog-password-symbolic", _("Share your PIN or address"), _("Share your PIN code or the server address so friends can connect.")))
        tips.append(_tip("security-high-symbolic", _("Keep it secure"), _("Keep your server and network secure. Use a VPN when sharing games.")))
        ...
        circle = Gtk.Box()
        circle.add_css_class("status-icon-circle")
        ...
        self.host_status_dot = create_icon_widget("media-record-symbolic", size=12, css_class=["status-dot", "status-offline"])
        ...
        self.tile_lat = MetricTile("network-transmit-receive-symbolic", _("Latency"), "metric-orange", (1.0, 0.45, 0.0, 1.0))
        self.tile_fps = MetricTile("video-display-symbolic", _("FPS"), "metric-green", (0.13, 0.76, 0.42, 1.0))
        self.tile_bw = MetricTile("network-wireless-symbolic", _("Bandwidth"), "metric-blue", (0.21, 0.52, 0.89, 1.0))
        ...
        footer_note.set_markup(_('<span size="small">Keep your server and network secure. Use a VPN when sharing games.</span>'))
    ...
    def open_pin_dialog(self, _widget: Gtk.Widget) -> None:
        ...
        dialog = Adw.MessageDialog(heading=_("Insert PIN"), body=_("Enter the PIN displayed on the client device (Moonlight)."))
    ...
    def prompt_create_user(self, pin_retry):
        dialog = Adw.MessageDialog(heading=_("User Not Found"), body=_("No user has been created in Sunshine. It is necessary to configure a user through the browser."))
        dialog.set_transient_for(self._root_window())
        dialog.add_response("cancel", _("Cancel"))
    ...
    def open_create_user_dialog(self, pin_retry):
        ...
        dialog = Adw.MessageDialog(heading=_("Create Sunshine User"), body=_("Define a username and password for Sunshine."))
        dialog.set_transient_for(self._root_window())
            ...
            if r == "save":
                ...
                if success:
                    ...
                    if ok:
                        ...
                        self.show_error_dialog(_("Error sending PIN after creation"), p_msg)
                else:
    ...
    def open_sunshine_auth_dialog(self, pin_to_retry: str, device_name: str | None = None):
        ...
        dialog = Adw.MessageDialog(heading=_("Sunshine Authentication"), body=_("Sunshine requires login. Enter your credentials (default: admin / password created during installation)."))
        dialog.set_transient_for(self._root_window())
    ...
    def sync_ui_state(self):
        if self.is_hosting:
            ...
            if hasattr(self, "start_btn_icon"):
                set_icon(self.start_btn_icon, "media-playback-stop-symbolic")
            ...
            for r in [self.game_mode_row, self.hardware_expander, self.streaming_expander, self.advanced_expander]:
            ...
            if hasattr(self, "start_btn_icon"):
                set_icon(self.start_btn_icon, "media-playback-start-symbolic")
    ...
    def _refresh_audio_mixer_ui(self):
        if not self.audio_mixer_expander.get_visible():
            return True
        if not hasattr(self, "audio_manager"):
            return True
            ...
            if mode_idx == 1:  # Steam
                ...
                if idx != Gtk.INVALID_LIST_POSITION:
                    ...
                    if 0 <= idx < len(games):
                        game = games[idx]
                        app_id = game.get("id", "")
                        self._game_launch_info = {"type": "steam", "app_id": app_id, "name": game["name"]}
                ...
                if idx != Gtk.INVALID_LIST_POSITION:
                    ...
                    if 0 <= idx < len(games):
                        game = games[idx]
                        self._game_launch_info = {"type": "lutris", "cmd": game["cmd"], "name": game["name"]}
            elif mode_idx == 3:  # Custom App
                name = self.custom_name_entry.get_text()
                cmd = self.custom_cmd_entry.get_text()
                if name and cmd:
                    self._game_launch_info = {"type": "custom", "cmd": cmd, "name": name}
    ...
    def _set_share_hint(self, ipv4):
        ...
        if ipv4 and ipv4 != "None":
            ...
            self.host_status_subtitle.set_label(_("Friends connect to this PC at {} — or use a PIN code.").format(ipv4))
        return False
    ...
    def _tick_uptime(self):
        ...
        mm, ss = divmod(rem, 60)
        self.host_uptime_label.set_label(_("Server active for {:02d}:{:02d}:{:02d}").format(hh, mm, ss))
    ...
    def _refresh_metric_tiles(self):
        ...
        self.tile_lat.update(norm([p.latency for p in history], chart.max_latency), chart._cur_latency_text)
        self.tile_fps.update(norm([p.fps for p in history], chart.max_fps), chart._cur_fps_text)
        self.tile_bw.update(norm([p.bandwidth for p in history], chart.max_bandwidth), chart._cur_bw_text)
    ...
    def open_paired_devices_dialog(self, _widget):
        dialog = Adw.MessageDialog(
            ...
            body=_("Devices paired with Sunshine. Disable to block access without re-pairing; remove to revoke (a new PIN will be required)."),
        )
        dialog.set_transient_for(self._root_window())
    ...
    def _unpair_all_devices(self):
        ...
        def work():
            ...
            GLib.idle_add(self.show_toast, _("Devices updated") if ok else _("Operation failed"))
    ...
    def open_game_library_dialog(self, _widget):
        if not self.sunshine.is_running():
            self.show_error_dialog(_("Server Not Running"), _("Start the server first to manage the game library."))
            return

        dialog = Adw.MessageDialog(
            heading=_("Game Library"),
            body=_("Games offered to guests in Moonlight. Add your detected games or remove entries."),
        )
        dialog.set_transient_for(self._root_window())
        dialog.add_response("close", _("Close"))
    ...
    def open_host_browse_dialog(self):
        if not self.sunshine.is_running():
            self.show_error_dialog(_("Server Not Running"), _("Start the server first to browse the host filesystem."))
            return
    ...
    def open_password_dialog(self, _widget):
        dialog = Adw.MessageDialog(
            ...
            body=_("Set the username and password used to manage the Sunshine server. If you forgot the current password, switch on “I forgot the current password” to reset it."),
        )
        dialog.set_transient_for(self._root_window())
        ...
        for r in (forgot_row, cur_user_row, cur_pass_row, new_user_row, new_pass_row, confirm_row):
            grp.add(r)
        dialog.set_extra_child(grp)
