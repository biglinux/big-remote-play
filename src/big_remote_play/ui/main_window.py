...
BASE_NAVIGATION_PAGES = {
    "welcome": {"name": _("Home"), "icon": "go-home-symbolic", "description": _("Home Page")},
    "host": {"name": _("Server"), "icon": "network-server-symbolic", "description": _("Share your games")},
    "guest": {"name": _("Connect to Server"), "icon": "network-workgroup-symbolic", "description": _("Connect to a host")},
    "section_private": {"name": _("Private Network"), "type": "separator"},
}
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def create_status_footer(self):
        ...
        def add_status_row(container, label_text, dot_attr, lbl_attr, service_id):
            ...
            row.update_property([Gtk.AccessibleProperty.LABEL, Gtk.AccessibleProperty.DESCRIPTION], [label_text, desc])
    ...
    def update_server_status(self, has_sun, has_moon, has_docker, has_tailscale, has_zt=False):
        for service_id, has in [("sunshine", has_sun), ("moonlight", has_moon), ("docker", has_docker), ("tailscale", has_tailscale), ("zerotier", has_zt)]:
    ...
    def create_vpn_selector_page(self):
        ...
        steps_group.add(
            create_steps_strip(
                [
                    ("document-save-symbolic", _("Install"), _("Install the chosen VPN provider on your device.")),
                    ("system-users-symbolic", _("Authenticate"), _("Log in and authorize your devices on the VPN.")),
                    ("input-gaming-symbolic", _("Play"), _("Connect to the server and play with your friends!")),
                ]
            )
        )
    ...
    def create_welcome_page(self):
        ...
        steps = create_steps_strip(
            [
                ("network-server-symbolic", _("Start or connect"), _("Start a server on this PC or connect to an existing one.")),
                ("system-users-symbolic", _("Invite friends"), _("Share the PIN code or the server address.")),
                ("input-gaming-symbolic", _("Play together"), _("Everyone connects and plays remotely.")),
            ]
        )
    ...
    def update_status(self, h_sun, h_moon):
        (self.show_missing_dialog() if not h_sun and not h_moon else None)

    def show_missing_dialog(self):
        d = Adw.MessageDialog.new(self)
        d.set_heading(_("Missing Components"))
        d.set_body(_("Sunshine and Moonlight are required. Install now?"))
        ...
        d.connect("response", lambda dlg, r: InstallerWindow(parent=self, on_success=self.check_system).present() if r == "install" else None)
        d.present()

    def show_toast(self, m):
        (self.toast_overlay.add_toast(Adw.Toast.new(m)) if hasattr(self, "toast_overlay") else print(m))
    ...
    def on_service_clicked(self, service_id):
        ...
        if service_id == "sunshine":
            is_running = self.system_check.is_sunshine_running()
        elif service_id == "moonlight":
            is_running = self.system_check.is_moonlight_running()
        elif service_id == "docker":
            is_running = self.system_check.is_docker_running()
        elif service_id == "tailscale":
            is_running = self.system_check.is_tailscale_running()
        elif service_id == "zerotier":
            is_running = self.system_check.is_zerotier_running()
        ...
        if service_id == "docker":
            ...
            c_dot = create_icon_widget("media-record-symbolic", size=12, css_class=["status-dot", "status-online" if cont_running else "status-offline"])
